...
import { checkIsValidEd25519Key, getRawPublicKey } from "../utils/utils.js";
const { webcrypto } = crypto;
/*
async function getRawPublicKey(publicKey) {
  console.log(publicKey);
  const spki_public_key = new Uint8Array(await webcrypto.subtle.exportKey('spki', publicKey)).slice(-32);

  // Currently this is the only way for us to get the raw 32 bytes of the public key.
  //const node_export = new Uint8Array(
  //  publicKey.export({ type: "spki", format: "der" }).slice(-32),
  //);
  //  spki_public_key.then((data, webcrypto_export = new Uint8Array(data).slice(-32)) => console.log({webcrypto_export, node_export}));
  return spki_public_key //node_export;
}
*/
export class IntegrityBlockSigner {
  ...
  async sign() {
    ...
    const newAttributes = {
      [ED25519_PK_SIGNATURE_ATTRIBUTE_NAME]: await getRawPublicKey(publicKey),
    };
  ...
  async calcWebBundleHash() {
    var hash = await webcrypto.subtle.digest("SHA-512", this.webBundle); // crypto.createHash('sha512');
    // var data = hash.update(this.webBundle);
    return new Uint8Array(hash); // new Uint8Array(data.digest());
  }
  ...
  async verifySignature(data, signature, publicKey) {
    ...
    // return webrypto.subtle.verify(alg, ed25519_key.publicKey, signature, data)
